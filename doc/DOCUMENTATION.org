...
| ~SPC b c~   | kill the current buffer (does not delete the visited file)               |
...
Text related commands (start with ~x~):
    ...
    | ~SPC x a c~ | align current alignment section using default rules           |
    | ~SPC x a r~ | align region using user-specified regexp                      |
    | ~SPC x a m~ | align region at arithmetic operators (+-*/)                   |
    | ~SPC x a x~ | align region (or guessed section) using a regex               |
